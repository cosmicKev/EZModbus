 ...
 * @return true if the timer is stopped (won't hit afterwards), false if queuing
 * command failed
 */
Client::PendingRequest::KillOutcome
Client::PendingRequest::killTimer(TickType_t maxWaitTicks) {
  ...
  if (xTimerStop(_timer, wait) != pdPASS) {
    ...
    return KillOutcome::FAILED; // Nothing queued -> no guarantee, timer will
...
void Client::PendingRequest::timeoutCallback(TimerHandle_t timer) {
  ...
  // One-liner RAII closing guard: ensure we call timerClosing(false) in all
  // return paths
  struct Closer {
    PendingRequest *p;
    ~Closer() { p->timerClosing(false); }
  } closer{pendingReq};
...

  {
    ...
    if (!pendingReq->_active) {
      Modbus::Debug::LOG_MSG(
          "Timeout callback: request already inactive, ignoring");
      return;
    }
  }
 ...
 * @param response Where to store the response (response.data will be resized to
 * match its size)
 * @param tracker Pointer to the transfer result tracker
 * @param timeoutMs Timeout in milliseconds
 * @return true if the pending request was set successfully, false is already
 * active (clear it first)
 */
bool Client::PendingRequest::set(const Modbus::Frame &request,
                                 Modbus::Frame *response, Result *tracker,
                                 uint32_t timeoutMs,
                                 EventGroupHandle_t waiterEventGroup) {
  if (isActive() || closingInProgress())
    return false; // Fail-fast

  Lock guard(_mutex);

  if (isActive() || closingInProgress())
    return false; // Avoid corruption & timer/response race
  ...
  if (!_timer) {
    ...
    _timer = xTimerCreate("ModbusTimeout", pdMS_TO_TICKS(timeoutMs), pdFALSE,
                          this, timeoutCallback);
#else
    _timer = xTimerCreateStatic("ModbusTimeout", pdMS_TO_TICKS(timeoutMs),
                                pdFALSE, this, timeoutCallback, &_timerBuf);
#endif
    ...
    if (xTimerChangePeriod(_timer, pdMS_TO_TICKS(timeoutMs),
                           TIMER_CMD_TOUT_TICKS) != pdTRUE) {
      _active = false;
      return false;
    };
  }
 ...
 * @param timeoutMs Timeout in milliseconds
 * @return true if the pending request was set successfully, false if already
 * active
 */
bool Client::PendingRequest::set(const Modbus::Frame &request,
                                 Client::ResponseCallback cb, void *userCtx,
                                 uint32_t timeoutMs) {
  if (isActive() || closingInProgress())
    return false; // Fail-fast
 ...
 */
bool Client::PendingRequest::hasResponse() const {
  return _pResponse != nullptr;
}
 ...
 */
const Modbus::FrameMeta &Client::PendingRequest::getRequestMetadata() const {
  return _reqMetadata;
}
 ...
 * @param fromTimer Internal flag - DO NOT USE from timer callback, use
 * setResultFromTimer() instead
 * @note ⚠️ DO NOT call this method from timer context! Use setResultFromTimer()
 * instead
 */
void Client::PendingRequest::setResult(Result result, bool finalize,
  ...
  // Atomic gate : we are closing the request if finalize is true and call
  // origins from response
  if (finalize && !fromTimer)
    respClosing(true);

  // One-liner RAII closing guard: ensure we call respClosing(false) in all
  // return paths
  struct Closer {
    PendingRequest *p;
    bool en;
  ...
  if (finalize && !fromTimer) {
    ...
    case KillOutcome::STOPPED_NOACK:
      Modbus::Debug::LOG_MSG(
          "Timer kill command sent but delayed, disarming callback...");
  ...
  // Handle result & tracker under critical section
  {
    ...
    // Re-open the gates: it's now safe to publish & let API callers launch a
    // new request (the _active flag & _mutex protect us until we leave this
    // function)
    if (fromTimer)
      timerClosing(false);
    else if (finalize)
  ...
  // Since we don't expect a response (failure or broadcast), we pass nullptr as
  // response
  if (cbSnapshot)
...
Client::Client(ModbusInterface::IInterface &interface, uint32_t timeoutMs)
    : _interface(interface), _requestTimeoutMs(timeoutMs),
      _pendingRequest(this), _isInitialized(false) {}
...
/* @brief Send a request to the interface (synchronous or asynchronous with
 * tracker)
 * @param request The request to send
 * @param response Where to store the response (response.data will be resized to
 * match its size)
 * @param userTracker nullptr for blocking (sync) mode, or pointer to transfer
 * result tracker for async mode
 * @return - In async mode: Success if the transfer was started successfully
 * (check the tracker to follow up)
...
Client::Result Client::sendRequest(const Modbus::Frame &request,
                                   ...
                                   Result *userTracker) {
  // Reject if the frame is not a request, invalid, or another request is
  // already in progress
  ...
  // Choose tracker to use : if userTracker is not provided ("sync mode"), we
  // create a local one
  Result localResult;
  ...
  if (!_pendingRequest.set(request, &response, tracker, _requestTimeoutMs,
                           syncEvtGrp)) {
    if (syncEvtGrp)
      vEventGroupDelete(syncEvtGrp); // Cleanup on failure
...
Client::Result Client::handleResponse(const Modbus::Frame &response) {
  ...
  if (!_interface.checkCatchAllSlaveIds() &&
      response.slaveId != reqSnapshot.slaveId) {
  ...
  _responseBuffer.regAddress =
      reqSnapshot.regAddress; // <- original address from snapshot
  _responseBuffer.regCount =
      reqSnapshot
          .regCount; // <- actual number of registers / coils from snapshot
 ...
 */
void Client::staticHandleTxResult(ModbusInterface::IInterface::Result result,
                                  void *pClient) {
  ...
  if (result != ModbusInterface::IInterface::SUCCESS ||
      Modbus::isBroadcastId(reqSnapshot.slaveId)) {
