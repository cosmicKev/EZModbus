...
// Default constructor
TCP::TCP()
    : _rxQueue(nullptr), _listenSocket(-1), _clientSocket(-1), _isServer(false), _isRunning(false),
      _cfgMode(CfgMode::UNINIT), _cfgPort(0)
{
...
bool TCP::beginClient(const char *serverIP, uint16_t port)
{
    ...
    {
        Modbus::Debug::LOG_MSGF("WARNING: Initial connection to %s:%u failed, will retry on first sendMsg", serverIP,
                                port);
...
void TCP::runTcpTask()
{
    ...
    while (_isRunning)
    {
        ...
        if (max_fd == -1)
        { // No sockets to monitor (e.g., client not connected yet, or server not started)
        ...
        if (!_isRunning)
            break; // Check flag again after select
        ...
        if (activity < 0)
        {
            ...
            {
                Modbus::Debug::LOG_MSGF("Too many select errors, sleeping %lums then retrying...",
                                        SELECT_RECOVERY_SLEEP_MS);
        ...
        { // Scope for socketsMutex lock
            Lock guard(_socketsMutex);
            if (!_isRunning)
                break; // Check flag again after acquiring lock
                ...
                if (new_socket >= 0)
                {
                    if (_activeSocketCount < MAX_ACTIVE_SOCKETS)
                    {
                        ...
                        if (flags != -1 && fcntl(new_socket, F_SETFL, flags | O_NONBLOCK) != -1)
                        {
                            ...
                            Modbus::Debug::LOG_MSGF("New connection from %s on socket %d. Active clients: %d",
                                                    client_ip, new_socket, static_cast<int>(_activeSocketCount));
                        }
                        else
                        {
                            Modbus::Debug::LOG_MSGF(
                                "Failed to set new client socket %d to non-blocking, errno: %d. Closing.", new_socket,
                                errno);
                            ::close(new_socket);
                        }
                    }
                    ...
                    {
                        Modbus::Debug::LOG_MSGF("Max clients (%d) reached. Rejecting new connection from socket %d.",
                                                MAX_ACTIVE_SOCKETS, new_socket);
                        ::close(new_socket);
                    }
                }
                else
            ...
            if (_isServer)
            {
                ...
                size_t max_sockets = std::min(_activeSocketCount, MAX_ACTIVE_SOCKETS);
            ...
            for (size_t idx = 0; idx < check_count; ++idx)
            {
                ...
                // Additional validation: check if socket is still valid before recv()
                // This helps prevent crashes from race conditions
                int error = 0;
                socklen_t len = sizeof(error);
                if (getsockopt(sock, SOL_SOCKET, SO_ERROR, &error, &len) < 0)
                {
                    Modbus::Debug::LOG_MSGF("Socket %d is no longer valid (getsockopt failed), errno: %d", sock, errno);
                    if (remove_count < MAX_ACTIVE_SOCKETS)
                    {
                ...
                if (error != 0)
                {
                    Modbus::Debug::LOG_MSGF("Socket %d has error %d, closing", sock, error);
                    if (remove_count < MAX_ACTIVE_SOCKETS)
                    {
                        sockets_to_remove[remove_count++] = sock;
                    }
                ...
                else
                {
                    ...
                    else if (recv_errno == EBADF)
                    {
                        // Socket is no longer valid (closed by another thread)
                        no_data_read = false; // Error = activity
                        Modbus::Debug::LOG_MSGF(
                            "recv(MSG_PEEK) on socket %d failed with EBADF - socket closed by another thread", sock);
                        if (remove_count < MAX_ACTIVE_SOCKETS)
                        {
                            sockets_to_remove[remove_count++] = sock;
                        }
                    }
                    else if (recv_errno == ECONNRESET || recv_errno == ENOTCONN || recv_errno == EPIPE)
                    {
                        // Connection lost
                        no_data_read = false; // Error = activity
                        Modbus::Debug::LOG_MSGF("recv(MSG_PEEK) error on socket %d, errno: %d (%s). Closing.", sock,
    ...
    if (_isServer)
    {
        for (size_t i = 0; i < _activeSocketCount; ++i)
        {
            if (_activeSockets[i] == sock)
            {
                ...
                --_activeSocketCount;
                Modbus::Debug::LOG_MSGF("Removed socket %d from active server clients. Count: %d", sock,
                                        static_cast<int>(_activeSocketCount));
                break;
            }
        }
    }
